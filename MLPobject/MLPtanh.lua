...
function MLP:addLayer(_neuronCount)
	...
	for i=1, self.network[layerIndex].neuronCount do
		...
		self.network[layerIndex][i].bias=0
...
function MLP:forwardProp(_inputs)
	...
	for layerIndex=firstHiddenLayerIndex, outputLayerIndex do
		for i=1, self.network[layerIndex].neuronCount do
			local sum=self.network[layerIndex][i].bias -- tinha um - aqui
			for j=1, self.network[layerIndex-1].neuronCount do
...
function MLP:save(fileName)
	...
	logIn.create(fileName)
	logIn.println("local MLP={}")
	
	local inputLayerIndex=1
	local firstHiddenLayerIndex=inputLayerIndex+1
	local outputLayerIndex=self.layerCount
	
	logIn.println("MLP.layerCount=" .. self.layerCount .. "")
	logIn.println("MLP.network={}")
	
	for layerIndex=inputLayerIndex, outputLayerIndex do
		logIn.println("MLP.network[" .. layerIndex .. "]={}")
		logIn.println("MLP.network[" .. layerIndex .. "].neuronCount=" .. self.network[layerIndex].neuronCount .. "")
	end
	
	for layerIndex=firstHiddenLayerIndex, outputLayerIndex do
		for i=1, self.network[layerIndex].neuronCount do
			logIn.println("MLP.network[" .. layerIndex .. "][" .. i .. "]={}")
			logIn.println("MLP.network[" .. layerIndex .. "][" .. i .. "].bias=" .. self.network[layerIndex][i].bias .. "")
			for j=1, self.network[layerIndex-1].neuronCount do
				logIn.println("MLP.network[" .. layerIndex .. "][" .. i .. "][" .. j .. "]=" .. self.network[layerIndex][i][j].weight)
			end
		end
	end
	
	logIn.println("return MLP")
	logIn.close()
end
